...
import { View, Text, StyleSheet, TouchableOpacity, ActivityIndicator, FlatList, Dimensions, Image, ScrollView, Modal } from 'react-native';
import { useFonts, Poppins_400Regular, Poppins_500Medium, Poppins_600SemiBold } from '@expo-google-fonts/poppins';
import { Lixo, Recicla, TitleCatalogo, MissIcon } from '../../assets';
...
const Catalogo = ({ navigation }) => {
	...
	// Função do filtro de categorias
	...
	// Função que capta 4 artigos do banco de dados
	const fetchArticles = async () => {
		try {
			...
			switch (response.status) {
				case 200:
					// Armazena os artigos com os mais recentes primeiro
					const sortedArticles = response.data.results.sort(
						(a, b) => b.pk_IDarticle - a.pk_IDarticle
					);
					
					// Modifica o estado das variáveis
			...
			if (error.response) {
				...
				switch (status) {
					case 500:
						...
						setModalErro(msg);
						console.error('Erro #500, /articles: ', response);
					break;

					default:
					showModal('Algo deu errado :(',  'Ocorreu um erro desconhecido. Tente novamente');
					console.error('Erro #default, /articles: ', response);
				}
				...
				console.error('Erro #???, /articles: ', response);
			}
		} finally {
	...
	//função que carrega mais artigos
		...
		if (loadedArticlesCount < totalArticles && !loadingMore) {
			...
			setTimeout(async () => {
				try{
					...
					switch (response.status) {
						case 200:
							// Armazena os 4 artigos retornados
